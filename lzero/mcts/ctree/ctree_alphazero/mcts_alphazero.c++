...
public:
    ...
    void _simulateV(std::shared_ptr<Node> V_sh, std::shared_ptr<Node> Q_sh, py::object simulate_env, py::object policy_value_func) {
        // Select next action
        int action;
        std::shared_ptr<Node> V_sh_plus_1 = nullptr;
        std::shared_ptr<Node> Q_sh_a = nullptr;
        
        std::tie(action, V_sh_plus_1, Q_sh_a) = _select_child(V_sh, Q_sh);
        
        // StimulateQ
        _simulateQ(action, V_sh_plus_1, Q_sh_a, simulate_env, policy_value_func);
        ...
        for (const auto& kv : Q_sh->children) {
            ...
            V_sh->value += (Q_sh_a_temp->visit_count / V_sh->visit_count) * pow(Q_sh_a_temp->value, p);
        }
        V_sh->value = pow(V_sh->value, 1 / p);
    }
    
    void _simulateQ(int action, std::shared_ptr<Node> V_sh_plus_1, std::shared_ptr<Node> Q_sh_a, py::object simulate_env, py::object policy_value_func) {
        // Apply action to env
        ...
        if (!done) {
            if (V_sh_plus_1->is_leaf()) {
                ...
                _simulateV(V_sh_plus_1, Q_sh_a, simulate_env, policy_value_func);
        ...
        leaf_value = std::max(0, (leaf_value + 1.0) / 2.0); 
